...
START_TEST(test_s21_maze_ml_3) {
  t_maze maze;
  int error = load_from_file("mazes/maze_20_20_1", &maze);
  if (!error) {
    t_point p_finish = {11, 7};
...
START_TEST(test_s21_maze_ml_3_close) {
  t_maze maze;
  int error = load_from_file("mazes/maze_20_20_1", &maze);
  if (!error) {
...
Suite *test_s21_maze_ml_20x20(void) {
  Suite *s = suite_create("\033[45m-=test_s21_maze_ml_20x20=-\033[0m");
  TCase *tc = tcase_create("test_s21_maze_ml_20x20");

  tcase_add_test(tc, test_s21_maze_ml_3);
  tcase_add_test(tc, test_s21_maze_ml_3_close);
